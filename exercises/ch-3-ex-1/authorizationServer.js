...
app.post("/token", function (req, res) {
  ...
  if (auth) {
    ...
    console.log("cliendSecret:", clientSecret);
  }
  ...
  if (req.body.client_id) {
    if (clientId) {
      ...
      res.status(401).json({ error: "invalid_client aaa" });
      return;
    }
  ...
  if (!client) {
    ...
    res.status(401).json({ error: "invalid_client  bbb" });
    return;
  }
  ...
  if (client.client_secret != clientSecret) {
    ...
    res.status(401).json({ error: "invalid_client ccc" });
    return;
  }
